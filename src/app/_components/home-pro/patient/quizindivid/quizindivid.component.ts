...
export class QuizindividComponent implements OnInit {
  ...
  Troubles_musculo_squelettiques =  new MedicalFileHistoryDto([], 'Troubles musculo-squelettiques (fracture)',false,[""])
  ...
  enregistrerQuiz(gender : string ,civilStatus : string ,familyIncome: string ,jobStatus: string ,livingEnvironment: string ,housingType: string
    ,frac1,frac2,frac3, education: string){
    let socio =  new SocioDemographicVariablesDto(civilStatus,familyIncome,jobStatus,education,livingEnvironment,housingType,gender);
    ...
    let ini = this.patient.firstName.substr(0,1);
    let tial = this.patient.lastName.substr(0,1);
    let initial = ini +""+tial;
    let fileNumber = this.patient.fileNumber;

    let individuQuiz = new IndividualQuestionnaireDto(this.patient.id,fileNumber,
      initial,JSON.stringify(socio),JSON.stringify(this.antecedants));
    let request = new Request(individuQuiz);
